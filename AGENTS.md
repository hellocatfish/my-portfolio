...
src/data/whitelist.json
  全身立绘点亮白名单。影响 CharacterImage 是否加载真实立绘。
...
- 先用 `isCharacterLit(name)` 判断 `whitelist.json`。
...
未点亮角色保持默认头像和 `stats: null`，不要删除简介和履历。
